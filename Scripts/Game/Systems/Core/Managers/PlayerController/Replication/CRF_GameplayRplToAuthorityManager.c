...
class CRF_GameplayRplToAuthorityManagerClass : ScriptComponentClass {}

class CRF_GameplayRplToAuthorityManager : ScriptComponent
{	
//=============================================================================================================================================================================================================================================================================================================================================================
//	 RUNTIME VARIABLES
//=============================================================================================================================================================================================================================================================================================================================================================
	...
	// LogAdminAction stayed on the lobby-side broadcast manager (used by its slotting/gear-swap RPCs too),
	// so admin-action logging from this class needs its own reference to that class.
...
//=============================================================================================================================================================================================================================================================================================================================================================
//	 CLIENT/RPC METHODS (CRF gameplay: rush/radio-extraction, mission save, admin tools/tickets/bug-reports, vehicle depot/supply, forward deploy, map markers, task creator, mini/sight arsenal, respawn settings, VAAR, positional sound)
//=============================================================================================================================================================================================================================================================================================================================================================
	...
	*/
	...
	protected void RpcAsk_RespawnFaction(FactionKey faction, bool logAction)
	{
		...
		if (logAction)
		{
			...
			m_LobbyBroadcastManager.LogAdminAction(logMessage, -1, false);
		}
	}
	...
	protected void RpcAsk_AddItem(int playerId, string prefab, bool logAction)
	{
		...
		if (logAction)
		{
			...
			m_LobbyBroadcastManager.LogAdminAction(logMessage, playerId, true);
		}
	...
	protected void RpcAsk_LogAdminAction(string data, int playerId, bool sendToPlayer)
	{
		...
		m_LobbyBroadcastManager.LogAdminAction(data, playerId, sendToPlayer);
	}
	...
	protected void RpcAsk_RequestPlayPositionalSound(string soundResource, string soundEvent, vector position)
	{
		...
		CRF_GameplayBroadcastManager broadcast = CRF_GameplayBroadcastManager.GetInstance();
		if (broadcast)
...
//=============================================================================================================================================================================================================================================================================================================================================================
	...
	protected static CRF_GameplayRplToAuthorityManager m_sInstance;
	void CRF_GameplayRplToAuthorityManager(IEntityComponentSource src, IEntity ent, IEntity parent)
	{
		m_sInstance = this;
	}

	//------------------------------------------------------------------------------------------------
	void ~CRF_GameplayRplToAuthorityManager()
	{
		if (m_sInstance == this)
			m_sInstance = null;
	}
	...
	//------------------------------------------------------------------------------------------------
	static CRF_GameplayRplToAuthorityManager GetInstance()
	{
		return m_sInstance;
	}
};
