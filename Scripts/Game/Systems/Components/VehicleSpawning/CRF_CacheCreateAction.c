class CRF_CacheCreateAction: ScriptedUserAction
{
	...
	override void PerformAction(IEntity pOwnerEntity, IEntity pUserEntity)
	{
		...
		RplId truckRplId;
		if (!COA_ReplicationHelper.GetRplId(m_Truck, truckRplId))
		{
		...
		RplId playerRplId;
		if (!COA_ReplicationHelper.GetRplId(controlledEntity, playerRplId))
		{
			Print("[CRF_CacheCreateAction] ERROR: Player entity has no RplComponent", LogLevel.ERROR);
			return;
		}
		
		COA_PlayerRplToAuthorityManager.GetInstance().CreateCache(truckRplId, playerRplId);
	}
