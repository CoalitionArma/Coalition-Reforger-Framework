class CRF_Frontline_HUD : SCR_InfoDisplayExtended
{
	...
	protected COA_PlayerControllerManager m_GameModePlayerComponent = null;
	...
	override protected void DisplayUpdate(IEntity owner, float timeSlice)
	{
		...
		if (!m_GameModePlayerComponent || !m_wASite || !m_wSiteCaptureBar || !m_wASiteInZone || !m_wASiteLock) {
			m_GameModePlayerComponent = COA_PlayerControllerManager.GetInstance();
		...
		if(COA_SafestartManager.GetInstance().GetSafestartStatus() || !SCR_BaseGameMode.Cast(GetGame().GetGameMode()).IsRunning() || !COA_PlayerControllerManager.GetInstance().m_bHUDVisible)
			m_wRoot.SetOpacity(0);
		else
