...
class CRF_FrontlineGamemodeManager: SCR_BaseGameModeComponent
{
	...
	void CheckAddInitialMarkers()
	{
		...
		UpdateMapMarkers(m_aZonesStatus, m_aZoneObjectNames, m_BluforSide, m_OpforSide);
	...
	void UpdateMapMarkers(array<string> zoneStatus, array<string> zoneObjectNames, FactionKey bluforSide, FactionKey opforSide)
	{
		...
		instance.RemoveALLScriptedMarkers();
		...
		foreach (int i, string zoneName : zoneObjectNames)
		{
			...
			if (zoneLocked == "Locked")
				instance.AddScriptedMarker(zoneName, "0 0 0", 0, "", "{91427B7866707601}UI\Objectives\lock.edds", 50, ARGB(255, 142, 142, 142));
			...
			// Add zone marker
			instance.AddScriptedMarker(zoneName, "0 0 0", 0, "", imageTexture, 45, imageColor);
		}
	}
