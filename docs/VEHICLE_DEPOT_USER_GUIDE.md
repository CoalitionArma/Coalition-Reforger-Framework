...
- Each prefab has a child SUPPLY STORAGE (Make sure to adjust supply count for des)
...
| 2 | **Basic Config** | Set Depot Name, Determine what COST types are needed |
| 3 | **Add Vehicles** | Vehicle Configuration → Add vehicles with Name, Prefab, Cost Type, Cost Amount |
| 4.A | **Configure Spawning** | Set Spawn Pattern, Distance, Spacing, Point Count or desired settings for spawning. |
| 4.B | **Configure Spawning: (Supplies)** | Based on COST type and mission needs, adjust the child Storage objects of the prafab and adjust starting SUPPLIES in **ResourceComponent**->**Container**. Or any other relevant settings.|
...
| **Made my own Depot, it doesnt detect supplies/cant be loaded into** | You need a child object with a properly configured resourcecomponent. Copy and paste the one(s) from the prefabs. |
