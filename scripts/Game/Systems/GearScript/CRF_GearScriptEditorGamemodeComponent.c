...
{	
	...
	void ExportGearScript(array<array<ResourceName>> gearArray)
	{
		...
		ResourceName GUIDObject = BaseContainerTools.CreateContainer("CRF_Clothing").GetResource().GetResourceName();
		Print(GUIDObject);
	...
	void AddToFinalWriteClothing(array<ResourceName> clothingArray, string GUID, string type)
	{
		if(clothingArray.Count() == 0)
			return;
		string clothingLine = string.Format("CRF_Clothing \"{%1}\" {", GUID);
		string clothingType = string.Format("m_sClothingType \"%1\"", type);
		ref array<string> finalArray = {};
		foreach (ResourceName item : clothingArray)
		{
			finalArray.Insert(string.Format("\"%1\"", item));
		}
		string clothingPrefabs = string.Format("m_ClothingPrefabs { %1 } }", SCR_StringHelper.Join(" ", finalArray, true));
		finalWriteArray.Insert(clothingLine);
		finalWriteArray.Insert(clothingType);
		finalWriteArray.Insert(clothingPrefabs);
	}
	*/
}